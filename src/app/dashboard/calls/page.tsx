...
export default async function CallsPage() {
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        <h1 className="text-lg font-semibold text-teal-900">Calls</h1>
      ...
      <div className="rounded-lg border border-stone-200 bg-white p-6">
        ...
        <ol className="mt-3 flex list-decimal flex-col gap-2 pl-5 text-sm text-stone-600">
      ...
      <div>
        <h2 className="mb-3 text-sm font-semibold text-teal-900">Call log</h2>
        {calls.length === 0 ? (
          ...
          <div className="overflow-hidden rounded-lg border border-stone-200 bg-white">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-stone-100">
                {calls.map((call) => (
                  <tr key={call.id}>
                    <td className="px-4 py-2 text-teal-900">{call.callerNumber}</td>
                    <td className="px-4 py-2 text-stone-600">{call.missed ? "Missed" : "Answered"}</td>
                    <td className="px-4 py-2 text-stone-600">{call.textSent ? "Yes" : "No"}</td>
